...
def test_desktop_runtime_report_is_built_and_loaded(tmp_path: Path) -> None:
    ...
    response = client.post(
        "/api/research/v1/desktop/runtime-check",
        params={"workspace_root": str(workspace_root)},
    )
    assert response.status_code == 200
    payload = response.json()
    assert payload["readiness_score"] >= 1
    assert payload["python_executable"]
    assert len(payload["checks"]) >= 5
    assert Path(payload["report_json_path"]).exists()

    latest_response = client.get(
        "/api/research/v1/desktop/runtime-check/latest",
        params={"workspace_root": str(workspace_root)},
    )
    assert latest_response.status_code == 200
    latest_payload = latest_response.json()
    assert latest_payload["workspace_root"] == str(workspace_root)
